...
from metapang.core.index.search import IndexSearch, SearchMode, SearchResult, IndexType, IndexBuilder
...
@click.option(
    "--mode", "-m",
    type=ClickEnum(SearchMode),
    help="Search mode"
)
@click.option(
    "--gather", "-g", is_flag=True, default=False,
...
@click.option(
    "--output", "-o",
    type=click.Path(path_type=str), default="{query_file}_{index_type}.tsv",
    help="Output file"
)
@click.option(
    "--threads",
...
@click.pass_context
def bank(ctx, index, pangbank, query, threshold_pangenome, threshold_genome, mode, gather, output, threads) -> None:
    """
    [bold]Search a bank[/]
    ...
    signature = IndexBuilder.file_batch_signature(
        query,
        search.info.kmer_size, search.info.scaled, search.info.n, True,
        threads, threads * 10, 1000,
    )

    if gather:
        import pandas as pd
        rows = search.gather_signature(signature, IndexType.genome, threshold=threshold_genome or 0.0)
        df = pd.DataFrame(rows, columns=["name", "f_query", "f_match"])
        if not df.empty:
            df["cum_f_query"] = df["f_query"].cumsum().round(4)
        out_path = f"{output.format(query_file=str(query), index_type='gather_genome')}.tsv"
    ...
    pang_results = search.search_signature(signature, IndexType.pangenome, mode, threshold_pangenome)
    geno_results = search.search_signature(signature, IndexType.genome, mode, threshold_genome)
    ...
    mp_log.info(f"Found {len(pang_results)} matches in pangenome index, writing to '{pang_output}'")
    mp_log.info(f"Found {len(geno_results)} matches in genome index, writing to '{geno_output}'")
