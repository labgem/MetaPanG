...
click.rich_click.OPTION_GROUPS["metapang index bank"] = [
    {"name": "Required", "options": ["--input", "--output"]},
    {
        "name": "Options",
        "options": ["--kmer-size", "--scaled", "--nb-hash", "--threads", "--help"],
    },
]
    ...
    if input_path.is_dir():
        ...
        for path in input_path.iterdir():
            if path.is_dir():
                fna = list(
                    chain.from_iterable(
                        path.rglob(ext)
                        for ext in ["*.fna*", "*.fa*", "*.fasta*", "*.gb*", "*.gbff*"]
                    )
                )
                if not fna:
        ...
        with open(input_path) as f:
            for n, line in enumerate(f):
                ...
                if len(line) < 2:
                    raise MetaPanG_IoError(
                        f"Invalid line in TSV file at line {n + 1} in {input_path}: '{line}'"
                    )
                if line[0] not in idx:
                    ...
                    raise MetaPanG_IoError(
                        f"Duplicate pangenome id '{line[0]}' at line {n + 1} in '{input_path}'"
                    )
    return pangenomes
...
def bank(
    input, output, kmer_size, scaled, nb_hash, genome_name_from_parent_dir, threads
) -> None:
    """
    ...
    with timer("Bank indexing") as t:
        ...
        mp_log.info(
            f"Index using k={kmer_size}, scaled={scaled}, and nb_hash={nb_hash} ({threads} threads)"
        )
