...
from sourmash.sbt import GraphFactory, SBT

from pyfastx import Fastx
from metapang.utils.io import silence, zopen, is_fastx
from metapang.utils.parallel import BoundedProcessPoolExecutor
from metapang.logger import mp_log_info, mp_log_error
...
def make_pangenome_signature_name(pangenome_id: str, genome_file: Path, use_parent_as_name: bool = False) -> str:
...
class IndexBuilder:
    ...
    @staticmethod
    def sequence_minhash(sequence: str, k: int, n: int, scaled: int, force: bool=True) -> MinHash:
    ...
    @staticmethod
    def add_sequence_minhash(mh: MinHash, sequence: str, k: int, n: int, scaled: int, force: bool=True) -> None:
    ...
    @staticmethod
    def add_sequences_minhash(mh: MinHash, sequences: Iterable[str], k: int, n: int, scaled: int, force: bool=True) -> None:
    ...
    @staticmethod
    def sequence_signature(name: str, sequence: str, k: int, n: int, scaled: int, force: bool=True) -> SourmashSignature:
        """Compute a Sourmash signature from a sequence."""
        return SourmashSignature(IndexBuilder.sequence_minhash(sequence, k, n, scaled, force), name=name)

    @staticmethod
    def sequence_batch_minhash(sequences: Iterable[str], k: int, n: int, scaled: int, force: bool=True) -> MinHash:
    ...
    @staticmethod
    def sequence_batch_signature(name: str, sequences: Iterable[str], k: int, n: int, scaled: int, force: bool=True) -> SourmashSignature:
        """Compute a Sourmash signature from a batch of sequences."""
        return SourmashSignature(IndexBuilder.sequence_batch_minhash(sequences, k, n, scaled, force), name=name)

    @staticmethod
    def file_minhash(file: Path, k: int, n: int, scaled: int, force: bool=True) -> MinHash:
    ...
    @staticmethod
    def file_signature(name: str, file: Path, k: int, n: int, scaled: int, force: bool=True) -> SourmashSignature:
        """Compute a Sourmash signature from a file (fasta/fastq/gzipped)."""
        return SourmashSignature(IndexBuilder.file_minhash(file, k, n, scaled, force), name=name)

    @staticmethod
    def pangenome_signature(name: str, genomes: List[Path], k: int, n: int, scaled: int, force: bool=True, use_parent: bool=False) -> Tuple[SourmashSignature, List[SourmashSignature]]:
        ...
        for genome in genomes:
            sg = IndexBuilder.file_signature(make_pangenome_signature_name(name, genome, use_parent), genome, k, n, scaled, force)
            m.merge(sg.minhash)
            signatures.append(sg)
        return SourmashSignature(m, name=name), signatures

    @staticmethod
    def add_signature_to_sbt(sbt: SBT, signatures: SourmashSignature | List[SourmashSignature]):
    ...
    @staticmethod
    def pangenome_construct(pangenome_id: str, genomes: List[Path], genome_sbt: SBT, pangenome_sbt: SBT, k: int, n: int, scaled: int, use_parent: bool=False) -> None:
        """Construct a pangenome signature and add it to the genome and pangenome SBTs."""
        pangenome_signature, genome_signatures = IndexBuilder.pangenome_signature(pangenome_id, genomes, k, n, scaled, use_parent=use_parent)
    ...
    @staticmethod
    def file_batch_signature(file: "Path | list[Path]", k: int, scaled: int, n: int, force: bool, nb_cores: int, queue_size: int, batch_size: int, with_log: bool=True) -> SourmashSignature:
        ...
        with BoundedProcessPoolExecutor(queue_size, max_workers=nb_cores, mp_context=_MP_CONTEXT, initializer=_ignore_sigint) as executor:
            try:
                sequences = []
                i = 0
                for f in files:
                    if is_fastx(f):
                        for record in Fastx(f):
                            ...
                            i += 1
                            if i % batch_size == 0:
                                executor.submit(IndexBuilder.add_sequences_minhash, m, sequences, k, n, scaled, force)
                                sequences = []
                    else:
    ...
    def construct(self, nb_cores: int, with_log: bool=True) -> IndexInfo:
        ...
        with silence():
            ...
            manager.register("SBT", SBT, exposed=("insert", "save", "_fill_min_n_below"))
            manager.start()
        ...
        with ProcessPoolExecutor(max_workers=nb_cores, mp_context=_MP_CONTEXT, initializer=_ignore_sigint) as executor:
            try:
                futures = {}
                for pangenome_id, genomes in self._config.pangenomes:
                    info.pangenomes[pangenome_id] = [g.stem if not self._config.use_parent_directory_as_genome_id else g.parent.stem for g in genomes]

                    f = executor.submit(IndexBuilder.pangenome_construct,
                                        pangenome_id, genomes, genome_sbt,
                                        pangenome_sbt, self._config.kmer_size,
                                        self._config.n, self._config.scaled,
                ...
                if with_log:
                    with Progress(*Progress.get_default_columns(), TimeElapsedColumn(), MofNCompleteColumn()) as progress:
                        task = progress.add_task("[bold]MetaPanG[Index][/]", total=len(futures))
        ...
        mp_log_info(with_log, f"Saving '{self._config.genome_index_name}' at '{self._config.output}'")
        genome_sbt.save(str(self._config.output / self._config.genome_index_name), sparseness=0.0)
        mp_log_info(with_log, f"Saving '{self._config.pangenome_index_name}' at '{self._config.output}'")
        pangenome_sbt.save(str(self._config.output / self._config.pangenome_index_name), sparseness=0.0)
